...
inline uint16_t get_rot_pixel(int16_t x, int16_t y){
   uint16_t angle = vgpu.output.angle;
   float flt_x = x * cos_deg(angle) + y * sin_deg(angle);
   float flt_y = y * cos_deg(angle) + x * sin_deg(angle);
   
   return vgpu.input.data[(int16_t)flt_y * vgpu.input.w + (int16_t)flt_x];
}

inline uint16_t get_scale_pixel(int16_t x, int16_t y){
...
void rotate(){
   for(int16_t cnt_y = 0; cnt_y < vgpu.output.h; cnt_y++){
      for(int16_t cnt_x = 0; cnt_x < vgpu.output.w; cnt_x++){
